...
function buscarComida(vida, miembros, mochila) {
  dialogo = alert(`total vida: ${vida}, miembros: ${miembros}, elementos en la mochila: ${mochila}`);
...
function lugarRefugio(vida, mochila) {
  dialogo = alert(`total vida: ${vida}, miembros: ${miembros}, elementos en la mochila: ${mochila}`);
...
function ayudarIgnorar(vida, miembros, mochila) {
  dialogo = alert(`total vida: ${vida}, miembros: ${miembros}, elementos en la mochila: ${mochila}`);
  let opcionAyudar = prompt("Mientras avanzan, escuchan gritos de auxilio desde un callejón. Ayudar podría ser un acto heroico... o un riesgo innecesario. ¿Cuál será su decisión? \n[A] Ayudar a una familia atrapada \n[B] Ignorar y seguir avanzando \n[C] Distraer a los enemigos y liberar a la familia a distancia \n[D] Robar los recursos de la familia sin ser vistos")
  if (opcionAyudar == "A") {
...
function señalHumo(mochila) {
  dialogo = alert(`total vida: ${vida}, miembros: ${miembros}, elementos en la mochila: ${mochila}`);
  let opcionHumo = prompt("Desde lo alto de un edificio, una columna de humo se eleva a lo lejos. ¿Es una señal de auxilio o una trampa mortal?. Elige \n[A] Investigar la señal \n[B] Evitar la zona")
  if (opcionHumo == "A") {
    alert("Encuentran gasolina y una llave vieja.");
    mochila.push("llave", "gasolina");
  } else if (opcionHumo == "B") {
    alert("Evitan una emboscada, pero pierden oportunidad de obtener objetos. Nada ocurre");
  } else {
    alert("Opción inválida");
  }
  return mochila;
...
function exploracion(vida, mochila) {
  dialogo = alert(`total vida: ${vida}, miembros: ${miembros}, elementos en la mochila: ${mochila}`);
  let opcionExploracion = prompt("El grupo necesita información. Las rutas están colapsadas, y moverse sin saber puede ser letal. ¿Mejor buscar una vista panorámica o seguir entre las sombras? \n[A] Subir a un edificio a observar [B] Moverse por callejones [C] Usar una alcantarilla conectada al metro")
...
function medioTransporte(vida, mochila) {
  dialogo = alert(`total vida: ${vida}, miembros: ${miembros}, elementos en la mochila: ${mochila}`);
  let opcionTransporte = prompt("La noche se acerca. Para avanzar rápido deben decidir: improvisar un medio de transporte o confiar en sus propias fuerzas. El tiempo es limitado. ¿Qué escoges? [A] Reparar una bicicleta [B] Ir a pie ")
